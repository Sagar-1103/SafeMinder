    ...
    const stopBackgroundService = async () => {
      ...
      const veryIntensiveTask = async (taskDataArguments) => {
          ...
          try {
              for (let i = 0; BackgroundService.isRunning(); i++) {
                  ...
                  if (i%15===0 && i>14) {
                      try {
                          await firestore().collection('Users').doc(code).update({userCurrentCoordinates:[location.latitude,location.longitude]});
                          await firestore().collection('Caretakers').doc(code).update({userCurrentCoordinates:[location.latitude,location.longitude]});
                          const res = await firestore().collection('Users').doc(code).get();
                          const tempRadius =await res._data.radius;
                          const centreLocation = await res._data.userHomeCoordinates;
      ...
      const outofboundshowNotification = () => {
        PushNotification.localNotification({
          ...
          title: `${user.name} : You are out of the Bound`,
...
//       <Text></Text>
//       <Button  title="All Ready Started Service ? " onPress={()=>navigation.navigate("Temp")} />
//       <Button title="Stop All Background Services" onPress={stopAllBackgroundServices} />
//     </View>
<><UserHome navigation={navigation} startAllBackgroundServices={startAllBackgroundServices} stopAllBackgroundServices={stopAllBackgroundServices}  /></>
  )
}